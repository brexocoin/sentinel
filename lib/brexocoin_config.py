...
class BrexocoinConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read brexocoin.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is brexocoin.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from brexocoin.conf
        ...
        # convert to an int if taken from brexocoin.conf
