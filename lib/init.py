...
def has_brexocoin_conf():
    import config
    import io

    valid_brexocoin_conf = False

    # ensure brexocoin_conf exists & readable
    #
    # if not, print a message stating that Brexocoin Core must be installed and
    # configured, including JSONRPC access in brexocoin.conf
    try:
        f = io.open(config.brexocoin_conf)
        valid_brexocoin_conf = True
    except IOError as e:
        print(e)

    return valid_brexocoin_conf
...
def main():
    ...
    if not has_brexocoin_conf():
        print("BrexocoinCore must be installed and configured, including JSONRPC access in brexocoin.conf")
        sys.exit(1)
