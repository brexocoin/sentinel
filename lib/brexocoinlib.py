...
def is_valid_brexocoin_address(address, network='mainnet'):
    ...
    brexocoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Brexocoin address
        return False

    if (address_version != brexocoin_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('brexocoin.conf')
