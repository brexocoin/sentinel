...
class BrexocoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some brexocoind methods
    ...
    @classmethod
    def from_brexocoin_conf(self, brexocoin_dot_conf):
        from brexocoin_config import BrexocoinConfig
        config_text = BrexocoinConfig.slurp_config_file(brexocoin_dot_conf)
        creds = BrexocoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = brexocoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = brexocoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = brexocoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_brexocoind_proto_version_with_sentinel_ping)
