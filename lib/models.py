...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, brexocoind):
        golist = brexocoind.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_brexocoind(brexocoind, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_brexocoind(self, brexocoind, rec):
        import decimal
        import brexocoinlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from brexocoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from brexocoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from brexocoind - Brexocoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(brexocoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from brexocoind! %s" % e)
            govobj.vote_delete(brexocoind)
            return (govobj, None)
    ...
    def vote_delete(self, brexocoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(brexocoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, brexocoind, signal, outcome):
        import brexocoinlib
        ...
        output = brexocoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = brexocoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(brexocoind, signal)

    def sync_network_vote(self, brexocoind, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = brexocoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = BREXOCOIND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 brexocoin addr, non-multisig
            if not brexocoinlib.is_valid_brexocoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Brexocoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Brexocoin Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (brexocoinlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = BREXOCOIND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not brexocoinlib.is_valid_brexocoin_address(addr, config.network):
    ...
    def hash(self):
        import brexocoinlib
        return brexocoinlib.hashit(self.serialise())
