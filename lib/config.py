...
sentinel_cfg = BrexocoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_brexocoind_proto_version_with_sentinel_ping = 70207


def get_brexocoin_conf():
    if sys.platform == 'win32':
        brexocoin_conf = os.path.join(os.getenv('APPDATA'), "BrexocoinCore/brexocoin.conf")
    else:
        home = os.environ.get('HOME')

        brexocoin_conf = os.path.join(home, ".brexocoin/brexocoin.conf")
        if sys.platform == 'darwin':
            brexocoin_conf = os.path.join(home, "Library/Application Support/BrexocoinCore/brexocoin.conf")

    brexocoin_conf = sentinel_cfg.get('brexocoin_conf', brexocoin_conf)

    return brexocoin_conf
...
brexocoin_conf = get_brexocoin_conf()
network = get_network()
